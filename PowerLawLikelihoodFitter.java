...
public class PowerLawLikelihoodFitter implements OptimizableFunction {
    ...
    public PowerLawLikelihoodFitter(double[] data, double[] samplingValues) {
	dimension = 3;
	this.data=data;
	...
	double[] paramLowerBounds = new double[] {0,0,2};
	double[] paramHigherBounds = new double[] {1e3,3,2};
	functionOptimizer.setMinima(paramLowerBounds);
	functionOptimizer.setMaxima(paramHigherBounds);

	// Optimization
	OptimizableFunction function = new PowerLawLikelihoodFitter(data, samplingValues);
	...
	System.out.println("  "+paramNames[2]+" = "+num.format(bestModel[2])+ " (fixed)");	
