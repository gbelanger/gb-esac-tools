...
public final class DataSmoother {
	...
	while ( k < (arrivalTimes.length - halfBlockSize) ) {
		...
		eventBlock[i] = arrivalTimes[k - halfBlockSize + i];
    ...
    public static double[] getGaussWeights(double[] xDistance, double[] yDistance, 
					   double xSigmaOfPSF, double ySigmaOfPSF) {
	...
	for ( int i=0; i < weight.length; i++ )
	    weight[i] = norm*Math.exp(-0.5 * (Math.pow(xDistance[i]/xSigmaOfPSF, 2) +
					      Math.pow(yDistance[i]/ySigmaOfPSF, 2)));
	return weight;
    }
